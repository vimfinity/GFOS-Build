...
const electronAPI = {
  ...
  scanModules: (pomPath: string) => ipcRenderer.invoke('scan:modules', pomPath),
  scanProfiles: (pomPath: string) => ipcRenderer.invoke('scan:profiles', pomPath),

  // Build
  startBuild: (job: BuildJob): Promise<boolean> => ipcRenderer.invoke('build:start', job),
  cancelBuild: (jobId: string): Promise<boolean> => ipcRenderer.invoke('build:cancel', jobId),

  // Events
  onBuildLog: (callback: (jobId: string, line: string) => void) => {
    const handler = (_event: IpcRendererEvent, jobId: string, line: string) => callback(jobId, line);
    ipcRenderer.on('build:log', handler);
    return () => ipcRenderer.removeListener('build:log', handler);
  },
  onBuildProgress: (callback: (jobId: string, progress: number) => void) => {
    const handler = (_event: IpcRendererEvent, jobId: string, progress: number) => callback(jobId, progress);
    ipcRenderer.on('build:progress', handler);
    return () => ipcRenderer.removeListener('build:progress', handler);
  },
  onBuildComplete: (callback: (jobId: string, status: string, exitCode: number | null) => void) => {
    const handler = (_event: IpcRendererEvent, jobId: string, status: string, exitCode: number | null) => 
  ...
  onBuildError: (callback: (jobId: string, error: string) => void) => {
    const handler = (_event: IpcRendererEvent, jobId: string, error: string) => callback(jobId, error);
  ...
  onScanStatus: (callback: (status: string | null) => void) => {
    const handler = (_event: IpcRendererEvent, status: string | null) => callback(status);
