...
function get256Color(n: number): string {
  ...
  if (n < 8) return ANSI_COLORS_DARK[n];
      ...
      if (seqEnd < line.length && line[seqEnd] === 'm') {
        ...
        while (ci < codes.length) {
          ...
          if (code === 0) {
            ...
            style.color = ANSI_COLORS_DARK[code - 30];
          } else if (code === 38) {
            // Extended foreground color
            const mode = codes[ci + 1];
            if (mode === 5 && ci + 2 < codes.length) {
              // 256-color: 38;5;N
              style.color = get256Color(codes[ci + 2]);
              ci += 2;
            } else if (mode === 2 && ci + 4 < codes.length) {
              // Truecolor: 38;2;R;G;B
              style.color = `rgb(${codes[ci + 2]}, ${codes[ci + 3]}, ${codes[ci + 4]})`;
              ci += 4;
            }
          } else if (code === 39) {
            style.color = undefined;
          } else if (code >= 40 && code <= 47) {
            style.bgColor = ANSI_COLORS_DARK[code - 40];
          } else if (code === 48) {
            ...
            style.bgColor = ANSI_BRIGHT_COLORS[code - 100];
          }
